...
export function BtcOscillators() {
  // ✅ 도메인을 포함한 절대 URL을 안전하게 생성
  const apiUrl = useMemo(() => {
    if (typeof window === "undefined") return null; // SSR 회피
    const u = new URL("/api/btc/market-chart", window.location.origin);
    u.searchParams.set("days", "120");
    return u.toString();
  }, []);

  const { data, error, isLoading } = useSWR<MarketChart>(
    apiUrl,                 // 키: window 준비되기 전엔 null (요청 안 감)
    apiUrl ? fetcher : null,
  ...
  const rsiPath = makeLinePath(rsiArr.filter(Number.isFinite), RSI_W, RSI_H);
  const macdPath = makeLinePath(macdLine.filter(Number.isFinite), MACD_W, MACD_H);
  const signalPath = makeLinePath(signalLine.filter(Number.isFinite), MACD_W, MACD_H);
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-4">
      ...
      <div className="rounded-lg border border-brand-line/30 bg-brand-card/60 p-3">
        <div className="text-sm font-medium flex items-center justify-between">
          ...
          <span className="text-brand-ink/70 text-xs">
            {typeof rsiLast === "number" ? Math.round(rsiLast) : isLoading ? "로딩…" : error ? "실패" : "-"}
          </span>
        </div>
        <div className="mt-2">
          {isLoading ? (
      ...
      <div className="rounded-lg border border-brand-line/30 bg-brand-card/60 p-3">
        <div className="text-sm font-medium flex items-center justify-between">
          <span title="MACD(12,26,9): MACD선이 시그널선을 상향 돌파하면 모멘텀 전환 신호">MACD (12,26,9)</span>
          <span className="text-brand-ink/70 text-xs">
            ...
            <>
              <svg width={MACD_W} height={MACD_H} className="w-full">
                {hist.map((v, i) => {
                  ...
                  const midY = MACD_H / 2;
                  const h = Math.min(Math.abs(v) * 2, MACD_H / 2);
